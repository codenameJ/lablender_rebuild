...
<v-row>
    <v-col cols="12">
        <div v-if="filterReceive.length == 0">
            ...
            <v-row>
                <span style="font-family:Prompt;" class="mx-auto subheading grey--text">ไม่มีคำขอยืมที่นักศึกษาได้รับอุปกรณ์มาแล้ว</span>
            </v-row>
        </div>
...
export default {
    ...
    computed: {
        ...
        filterReceive() {
            let readylist =
                this.StudentRequest.filter(list => list.status == "receive") || {};
            return readylist;
        }
    },
    watch: {}
